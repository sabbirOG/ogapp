...
const focusOptions = ['General wellbeing', 'Productivity', 'Health and habits', 'Study and learning', 'Faith and routine', 'Money management']

function FocusField({ name, defaultValue, required = false }: { name: string; defaultValue?: string; required?: boolean }) {
  const [value, setValue] = useState(defaultValue || '')
  const [open, setOpen] = useState(false)
  const suggestions = focusOptions.filter((option) => option.toLowerCase().includes(value.toLowerCase()))

  return <div className="focus-field"><input required={required} name={name} value={value} placeholder="Choose or write what matters to you" onFocus={() => setOpen(true)} onChange={(event) => { setValue(event.target.value); setOpen(true) }} onBlur={() => window.setTimeout(() => setOpen(false), 120)} />{open && suggestions.length > 0 && <div className="focus-suggestions" role="listbox">{suggestions.map((option) => <button type="button" key={option} onMouseDown={(event) => event.preventDefault()} onClick={() => { setValue(option); setOpen(false) }}>{option}</button>)}</div>}</div>
}

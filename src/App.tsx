...
function App() {
  ...
  useEffect(() => {
    const checkReminders = () => {
      ...
      const now = new Date()
      tasks.forEach((task) => {
        const reminderKey = `${task.id}-${now.toISOString().slice(0, 10)}`
  ...
  const runCommand = () => {
    ...
    if (lower.startsWith('expense ') || lower.startsWith('spent ')) {
      ...
      const title = cleaned.replace(/\bat\s+(?:\d{1,2}(?::\d{2})?\s*(?:am|pm)|(?:[01]\d|2[0-3]):[0-5]\d)\b/i, '').trim()

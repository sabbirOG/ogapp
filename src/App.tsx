...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav>{['Today', 'Tasks', 'Habits', 'Money', 'Notes', 'Goals'].map((item) => <button className={active === item ? 'nav-item active' : 'nav-item'} key={item} onClick={() => setActive(item)}><span className="nav-icon">{item.slice(0, 1)}</span>{item}</button>)}</nav>
...
function BottomNav({ active, onChange }: { active: string; onChange: (page: string) => void }) { return <nav className="bottom-nav">{['Today', 'Tasks', 'Habits', 'Money', 'Notes'].map((item) => <button className={active === item ? 'bottom-nav-item active' : 'bottom-nav-item'} key={item} onClick={() => onChange(item)}><span>{item.slice(0, 1)}</span>{item}</button>)}<button className={active === 'Goals' ? 'bottom-nav-item active' : 'bottom-nav-item'} onClick={() => onChange('Goals')}><span>G</span>Goals</button></nav> }
